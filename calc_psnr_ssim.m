function Evaluate_PSNR_SSIM()
    ...
    %% set path
    ext = {'*.jpg', '*.png', '*.bmp'};
    record_results_txt = ['result.txt'];
    results = fopen(fullfile(record_results_txt), 'wt');

    num_imgs = length(dir('./tmp/HR'))-2;

    PSNR_all = zeros(1, num_imgs);
    SSIM_all = zeros(1, num_imgs);
    for idx_im = 1:num_imgs
        im_HR = imread(fullfile('./tmp/HR', [num2str(idx_im-1), '.png']));
        im_SR = imread(fullfile('./tmp/SR', [num2str(idx_im-1), '.png']));
        ...
        % calculate PSNR, SSIM
        [PSNR_all(idx_im), SSIM_all(idx_im)] = ...
            Cal_Y_PSNRSSIM(im_HR_Y*255, im_SR_Y*255);
    end
    fprintf(results, '%f %f', mean(PSNR_all), mean(SSIM_all));
    fclose(results);

    end

    function [psnr_cur, ssim_cur] = Cal_Y_PSNRSSIM(A,B)
    % RGB --> YCbCr
    if 3 == size(A, 3)
        A = rgb2ycbcr(A);
        A = A(:,:,1);
